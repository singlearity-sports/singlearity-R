...
test_that("test-pa_pred_bases_loaded", {
  ...
  expect_lt(results[1,]$ibb_exp, 1e-05)
  
  # double plays, overall
  
  expect_lt(results[1,]$dp_exp, 1e-05)
  
  # double plays, ground balls
  
  expect_lt(results[1,]$gdp_exp, 1e-05)
  
  # sacrifice flies, overall
  
  expect_lt(results[1,]$sf_exp, 1e-05)
  
  # sacrifice fly double plays
  
  expect_lt(results[1,]$sf_dp_exp, 1e-05)
  
  # sacrifice hits
  
  expect_lt(results[1,]$sh_exp, 1e-05)
  
  # strikeout double plays
  
  expect_lt(results[1,]$so_dp_exp, 1e-05)
  
  # triple plays
  
  expect_lt(results[1,]$tp_exp, 1e-05)
